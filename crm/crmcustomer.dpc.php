...
class crmcustomer extends crmmodule  {
	...
	public function customer_grid($width=null, $height=null, $rows=null, $mode=null, $noctrl=false) {
	    ...
	    if (defined('MYGRID_DPC')) {
			...
			GetGlobal('controller')->calldpc_method("mygrid.column use grid2+code2|".localize('_code2',getlocal())."|10|0||||1|");
			GetGlobal('controller')->calldpc_method("mygrid.column use grid2+name|".localize('_name',getlocal())."|link|30|"."javascript:showdetails(\"{code2}\");".'||');
			...
			GetGlobal('controller')->calldpc_method("mygrid.column use grid2+address|".localize('_address',getlocal())."|30|1|");
			...
			$ret .= GetGlobal('controller')->calldpc_method("mygrid.grid use grid2+customers+$xsSQL2+$mode+$title+id+$noctrl+1+$rows+$height+$width+1+1+1");
	...
	protected function address_grid($width=null, $height=null, $rows=null, $mode=null, $noctrl=false) {
	    ...
	    if (defined('MYGRID_DPC')) {
			...
			$ret .= GetGlobal('controller')->calldpc_method("mygrid.grid use grid2+custaddress+$xsSQL2+$mode+$title+id+$noctrl+1+$rows+$height+$width+1+1+1");
	...
	protected function user_grid($width=null, $height=null, $rows=null, $mode=null, $noctrl=false) {
	    ...
	    if (defined('MYGRID_DPC')) {
			...
			GetGlobal('controller')->calldpc_method("mygrid.column use grid1+username|".localize('_username',getlocal())."|10|1|");						
			...
			$ret .= GetGlobal('controller')->calldpc_method("mygrid.grid use grid1+user+$xsSQL+$mode+$title+id+$noctrl+1+$rows+$height+$width+1+1+1");
