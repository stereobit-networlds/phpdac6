...
$__LOCALE['CRMDOCS_DPC'][0]='CRMDOCS_DPC;Tasks;Εργασίες';
...
class crmdocs extends crmmodule  {
	...
	public function docs_grid($width=null, $height=null, $rows=null, $mode=null, $noctrl=false) {
	    ...
	    if (defined('MYGRID_DPC')) {
			...
			//echo $xSQL2;
			GetGlobal('controller')->calldpc_method("mygrid.column use grid3+id|".localize('_id',getlocal())."|2|0|");
			GetGlobal('controller')->calldpc_method("mygrid.column use grid3+date|".localize('_date',getlocal())."|5|1|");//"|link|5|"."javascript:showdetails({id});".'||');
			GetGlobal('controller')->calldpc_method("mygrid.column use grid3+docid|".localize('docid',getlocal())."|10|1||||1|");			
			GetGlobal('controller')->calldpc_method("mygrid.column use grid3+cid|".localize('_user',getlocal())."|10|1||||1|");
			GetGlobal('controller')->calldpc_method("mygrid.column use grid3+formname|".localize('form',getlocal())."|link|5|"."javascript:showdetails(\"{cid}\");".'||');						
			GetGlobal('controller')->calldpc_method("mygrid.column use grid3+title|".localize('_subject',getlocal())."|link|19|"."javascript:showdetails(\"{docid}\");".'||');//."|19|0|");
			GetGlobal('controller')->calldpc_method("mygrid.column use grid3+timeout|".localize('_date',getlocal())."|5|1|");
		    GetGlobal('controller')->calldpc_method("mygrid.column use grid3+reply|".localize('_reply',getlocal())."|2|0|");	
			GetGlobal('controller')->calldpc_method("mygrid.column use grid3+status|".localize('_status',getlocal())."|2|0|||||right");	
		    GetGlobal('controller')->calldpc_method("mygrid.column use grid3+mailstatus|".localize('_failed',getlocal())."|2|1|");			
			
			$ret .= GetGlobal('controller')->calldpc_method("mygrid.grid use grid3+crmdocs+$xSQL2+$mode+$title+id+$noctrl+1+$rows+$height+$width+1+1+1");
	...
	protected function clicks_grid($width=null, $height=null, $rows=null, $mode=null, $noctrl=false) {
	    ...
	    if (defined('MYGRID_DPC')) {
			...
			GetGlobal('controller')->calldpc_method("mygrid.column use grid9+date|".localize('_date',getlocal()).'|10|0|');				
            GetGlobal('controller')->calldpc_method("mygrid.column use grid9+tid|".localize('_tid',getlocal()).'|5|0|'); 
            GetGlobal('controller')->calldpc_method("mygrid.column use grid9+attr1|".localize('_attr',getlocal()).'|19|0|');
            ...
            GetGlobal('controller')->calldpc_method("mygrid.column use grid9+ref|".localize('_cid',getlocal()).'|10|0|');			
			GetGlobal('controller')->calldpc_method("mygrid.column use grid9+REMOTE_ADDR|".localize('_ip',getlocal())."5|0|");//"|link|10|"."javascript:showdetails(\"{REMOTE_ADDR}\");".'||');
	...
	protected function docitems_grid($width=null, $height=null, $rows=null, $mode=null, $noctrl=false) {
	    ...
	    if (defined('MYGRID_DPC')) {
			...
			//GetGlobal('controller')->calldpc_method("mygrid.column use grid1+docid|".localize('docid',getlocal())."|1|0|");		
			//GetGlobal('controller')->calldpc_method("mygrid.column use grid1+name|".localize('_title',getlocal()). "|5|0|");
			GetGlobal('controller')->calldpc_method("mygrid.column use grid1+act|".localize('_active',getlocal())."|boolean|1|");
			...
			GetGlobal('controller')->calldpc_method("mygrid.column use grid1+code5|".localize('_code',getlocal())."|link|4|"."javascript:showdetails(\"{itemcode}~$selected\");".'||');
			...
			GetGlobal('controller')->calldpc_method("mygrid.column use grid1+price0|".localize('_price',getlocal())."|10|1|");	
			GetGlobal('controller')->calldpc_method("mygrid.column use grid1+price1|".localize('_price',getlocal())."|2|1|");				
			$ret .= GetGlobal('controller')->calldpc_method("mygrid.grid use grid1+crmdocsitem+$xsSQL2+$mode+$title+id+$noctrl+1+$rows+$height+$width+1+1+1");
