...
class reservations { 
	...
	function __construct() {
		...
		if (($UserName) && (($id) || ($incart))) {
            ...
            //DISABLED, NO ROUTE, USED IN XIXUSER   			
			//$this->get_map_javascript(); 
			
			//DISABLE PHOCAL, XIXUSER_DPC MUST BE LOADED
			/*if (!defined('XIXUSER_DPC')) {//..g v3 map			
			    //DISABLED, USE JS FROM XIXUSER
				$this->get_phocal_map_javascript(); 
			}*/	
...
GPLUSPOST;
	...
	/*protected function get_map_javascript() {
	
       if (iniload('JAVASCRIPT')) {
	   
	       $code = $this->map_javascript();	   	
		   $js = new jscript;		   	 	
		   $js->load_js('http://maps.google.com/maps/api/js?sensor=false',null,null,null,true);		   		   
		   //$js->load_js('http://www.google.com/jsapi?key=ABQIAAAAnkzS5k7dnQ2uLxfwaFLl2hRpSleFYPnkQ8Y_C3jn7GzY8FTUKhT9qbfeUm_--MkivYEktvCm4RNCIQ',null,null,null,true);	
           $js->load_js($code,null,1);		
		   unset ($js);
	   }	
	}	
	
	protected function map_javascript($header=false)  {
	    $geolanguage = getlocal() ? 'el' : 'en';
				
	    if ($header)
			$jmapscript = '<script type="text/javascript" >//<![CDATA[';
...
function showPosition(position) {
  //alert('xaxaxxx'+position.coords.latitude);
  document.getElementById('user_latitude_input').value = position.coords.latitude;
  document.getElementById('user_longitude_input').value = position.coords.longitude;  
};
...
function showError(error)  {
  switch(error.code) 
    ...
    case error.PERMISSION_DENIED:
      ugeo.innerHTML="User denied the request for Geolocation."
      break;
    case error.POSITION_UNAVAILABLE:
      ugeo.innerHTML="Location information is unavailable."
      break;
    case error.TIMEOUT:
      ugeo.innerHTML="The request to get user location timed out."
      break;
    case error.UNKNOWN_ERROR:
      ugeo.innerHTML="An unknown error occurred."
      break;
    }
};		
USERMAP;
	...
	/*protected function get_phocal_map_javascript() {
	
       if (iniload('JAVASCRIPT')) {
	   
	       $code = $this->map_phocal_javascript();	   	
		   $js = new jscript;		   	 	
		   //$js->load_js('http://maps.google.com/maps/api/js?sensor=false',null,null,null,true);		   		   
		   $js->load_js('http://www.google.com/jsapi?key=ABQIAAAAnkzS5k7dnQ2uLxfwaFLl2hRpSleFYPnkQ8Y_C3jn7GzY8FTUKhT9qbfeUm_--MkivYEktvCm4RNCIQ',null,null,null,true);	
           $js->load_js($code,null,1);		
	...
	protected function map_phocal_javascript($header=false) {
	    $keep_id = GetReq('id') ? 'id='.GetReq('id').'&cat='.GetReq('cat') : 'cat='.GetReq('cat');
	    $ajaxurl = seturl($keep_id."&t=");
        $geolanguage = getlocal() ? 'el' : 'en';		
	
	    if ($header)
			$jsmap = '<script type="text/javascript" >//<![CDATA[';
	
	    $jsmap = '';
	
	    $jsmap .= <<<PHOCALMAP
		
google.load("maps", "2.x", {"language" : "{$geolanguage}"});
google.load("search", "1", {"language" : "{$geolanguage}"});
...
function CheckPhocaMap(drive, iproject) {
	var tstPhocaMap=document.getElementById('phoca_map');
...
function getPhocaMap(drive, iproject,lat,lon){
	var tstPhocaMap=document.getElementById('phoca_map');
    lat= lat ? lat : document.getElementById('user_latitude_input').value;
    lon= lon ? lon : document.getElementById('user_longitude_input').value; 
    var iproject = parseInt(iproject);
	...
	var toPMAddress = '41.3333,22.3333';		 		 
    var fromPMAddress = lat+','+lon;	
	var drive = parseInt(drive);
	mydrive = drive ? drive : 0;

	if (tstPhocaMap.offsetWidth > 0) {
	
		mapPhocaMap = new GMap2(document.getElementById('phoca_map'));
		dirPhocaMap = new GDirections(mapPhocaMap, document.getElementById("phoca_dir"));
		//GEvent.addListener(dirPhocaMap, "error", handleErrors);
		mapPhocaMap.setCenter(new GLatLng(lat,lon), 17);
		mapPhocaMap.enableContinuousZoom();
		mapPhocaMap.enableDoubleClickZoom();
		mapPhocaMap.enableScrollWheelZoom();
		mapPhocaMap.addControl(new GLargeMapControl3D());
		mapPhocaMap.addControl(new GMapTypeControl());

		var point1 = new GPoint( lon,lat);
		var markerPhocaMarker1 = new GMarker(point1, {title:"XIX"});
		mapPhocaMap.addOverlay(markerPhocaMarker1);
					
		GEvent.addListener(markerPhocaMarker1, 'click', function() {
			markerPhocaMarker1.openInfoWindowHtml('<h1>XIX</h1><div><p class=\"info\"><img src=\"images/logo.png\" /></p></div>');
		});
		...
		if ((mydrive > 0) && (iproject)) {
		    ...
		    document.getElementById("phoca_dir").innerHTML = '';
		
			$.get('{$ajaxurl}getprojectlocation&iproject='+iproject, function(data)
	        {
				dirPhocaMap.load("from: " + fromPMAddress + " to: " + data, {"language" : "{$geolanguage}"});
	        });	
		}	
		else
			document.getElementById("phoca_dir").innerHTML = '';
...
function setDirections(drive, iproject) {
	var tstPhocaMap=document.getElementById('phoca_map');
	tstPhocaMap.style.height='250px';
	tstPhocaMap.style.width='100%';
	var tstIntPhocaMap;
	var drive = parseInt(drive);
	mydrive = drive ? drive : 0;
	var iproject = parseInt(iproject);
	...
	if (GBrowserIsCompatible()) {
		...
		tstIntPhocaMap=setInterval("CheckPhocaMap(mydrive, myproject)",500);
	}	
};	
PHOCALMAP;
...
EOF;
	...
	public function render() {
	    ...
	    if (($this->get_data($iproject)) && (isset($_GET['week'])) && ($id)) {
			...
			$days_row.= ($iproject>0) ? '' : '<input type="button" class="blue_button small_button" id="reservation_today_button" value="'.$today.'">';
			$days_row.= '<input type="button" class="blue_button small_button" id="reservation_hide_button" value="'.$close.'">'.
	...
	function show_xixuser() {
        ...
        /*else { //phocal map
		    $user_location_button = "<button class='btn' onclick='setDirections()'>{$update_geo_button}</button>";
			if ($iproject)
				$project_directions_button = "<button class='btn' onclick='setDirections(1,$iproject)'>{$find_geo_button}</button>";
			
			$mapholder = "<div id='phoca_map'></div><div id='phoca_dir'></div>";
        }*/				
	...
	<hr>	
	<h3>{$detailslabel}</h3>
	<p class='smalltext_p'>{$detailstext}			
	$user_location_button
	$project_directions_button
	</p>	
    $mapholder
...
XIXFORM;
	...
	protected function change_user_details($user_name, $user_longitude, $user_latitude) {
		...
		/*if($this->validate_user_email($user_email) != true) {
			return('<span class="error_span">Email must be a valid email address and be no more than 50 characters long</span>');
		}*/
		/*if($this->validate_user_password($user_password) != true && !empty($user_password)) {
		...
		/*if($this->user_email_exists($user_email) == true && $user_email != $_SESSION['user_name']) {
		...
		if ($project_id) {
			...
			foreach ($result as $n=>$rec) {
				...
				$active_deleted = $rec['active'] ? 
				                  /*localize('_active',getlocal())*/"<li class='icon-check'/>" : 
				                 ($rec['deleted'] ? /*localize('_deleted',getlocal())*/"<li class='icon-circle'/>" : "<li class='icon-circle'/>");
