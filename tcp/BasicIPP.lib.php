...
class ippException extends Exception
{
 ...
 {
  $return = sprintf("[ipp]: %s -- " . self::_dummy(" file %s, line %s"),
...
class BasicIPP
{
 ...
 public function setPrinterURI($uri) 
 {
  ...
  self::_putDebug(sprintf(self::_dummy("Printer URI: %s") , $uri) , 2);
  $this->setup->uri = 1;
 }
 ...
 public function setRawText() 
 {
  ...
  self::_putDebug(self::_dummy("Forcing data to be interpreted as RAW TEXT") , 2);
 }
 ...
 public function unsetRawText() 
 {
  ...
  $this->datahead = '';
  $this->datatail = '';
  self::_putDebug(self::_dummy("Unset forcing data to be interpreted as RAW TEXT") , 2);
 }
 ...
 public function setCharset($charset = 'us-ascii') 
 {
  ...
  $this->meta->charset = chr(0x47) // charset type | value-tag
   ...
   . $charset; // value
  self::_putDebug(sprintf(self::_dummy("Charset: %s") , $charset) , 2);
 ...
 public function setLanguage($language = 'en_us') 
 {
  ...
  $this->meta->language = chr(0x48) // natural-language type | value-tag
   ...
   . $language; // value
  self::_putDebug(sprintf(self::_dummy("Language: %s") , $language) , 2);
 ...
 public function setDocumentFormat($mime_media_type = 'application/octet-stream') 
 {
  ...
  while (strlen($length) < 2) $length = chr(0x00) . $length;
  self::_putDebug(sprintf(self::_dummy("mime type: %s") , $mime_media_type) , 2);
 ...
 public function setCopies($nbrcopies = 1) 
 {
  ...
  $this->meta->copies = chr(0x21) // integer type | value-tag
   ...
   . $copies;
  self::_putDebug(sprintf(self::_dummy("Copies: %s") , $nbrcopies) , 2);
  $this->setup->copies = 1;
 }
 ...
 public function setDocumentName($document_name = "") 
 {
  ...
  $length = chr($length);
  while (strlen($length) < 2) $length = chr(0x00) . $length;
  self::_putDebug(sprintf(self::_dummy("document name: %s") , $document_name) , 2);
 ...
 public function setJobName($jobname = '', $absolute = false) 
 {
  ...
  $this->meta->jobname = chr(0x42) // nameWithoutLanguage type || value-tag
   ...
   . $jobname; // value
  self::_putDebug(sprintf(self::_dummy("Job name: %s") , $jobname) , 2);
 ...
 public function setUserName($username = 'PHP-SERVER') 
 {
  ...
  $this->meta->username = chr(0x42) // keyword type || value-tag
   ...
   . $username;
  self::_putDebug(sprintf(self::_dummy("Username: %s") , $username) , 2);
 ...
 public function setAuthentication($username, $password) 
 {
  ...
  self::_putDebug(self::_dummy("Setting password") , 2);
 ...
 public function setSides($sides = 2) 
 {
  ...
  self::_putDebug(sprintf(self::_dummy("Sides value set to %s") , $sides) , 2);
 }
 ...
 public function setFidelity() 
 {
  ...
  $this->meta->fidelity = chr(0x22) // boolean type  |  value-tag
   ...
   . chr(0x01); //  true | value
  self::_putDebug(self::_dummy("Fidelity attribute is set (paranoid mode)") , 3);
 }
 ...
 public function unsetFidelity() 
 {
  ...
  self::_putDebug(self::_dummy("Fidelity attribute is unset") , 2);
 }
 ...
 public function setMessage($message = '')
 {
  ...
  self::_putDebug(sprintf(self::_dummy('Setting message to "%s"') , $message) , 2);
 }
 ...
 public function setAttribute($attribute, $values) 
 {
  ...
  {
   trigger_error(
     sprintf(self::_dummy('SetAttribute: Tag "%s" is not a printer or a job attribute'),
 ...
 public function unsetAttribute($attribute) 
 {
  ...
  {
   trigger_error(
     sprintf(self::_dummy('unsetAttribute: Tag "%s" is not a printer or a job attribute'),
      $attribute) , E_USER_NOTICE);
   self::_putDebug(
     sprintf(self::_dummy('unsetAttribute: Tag "%s" is not a printer or a job attribute'),
      $attribute) , 3);
 ...
 public function printJob() 
 {
  ...
  if (is_readable($this->data)) 
  {
   self::_putDebug(self::_dummy("Printing a FILE"));
  ...
  {
   self::_putDebug(self::_dummy("Printing DATA"));
   $this->output = 
    $this->stringjob
    . $this->datahead
    . $this->data
 ...
 protected function _sendHttp($post_values, $uri) 
 {
  ...
  self::_putDebug(self::_dummy("Processing HTTP request") , 2);
  $this->serveroutput->headers = array();
  $this->serveroutput->body = "";
  $http = new httpclient; //http_class;
...
"job-state" => array(
 ...
 protected function _parseHttpHeaders() 
 {
  ...
  switch ($this->serveroutput->headers[0]) 
  {
   ...
   trigger_error(
     sprintf(self::_dummy("server responds %s") , $server_response),
     E_USER_WARNING);
 ...
 protected function _stringJob() 
 {
  ...
  if (!isset($this->setup->uri)) 
  {
   ...
   {
    trigger_error(
      self::_dummy("_stringJob: Printer URI is not set: die"),
      E_USER_WARNING);
    self::_putDebug(self::_dummy("_stringJob: Printer URI is not set: die") , 4);
  ...
  self::_putDebug(
    sprintf(self::_dummy("String sent to the server is: %s"),
     $this->stringjob)
          );
  return TRUE;
 }
 ...
 protected function _giveMeStringLength($string) 
 {
  ...
  {
   $errmsg = sprintf (
     self::_dummy('max string length for an ipp meta-information = %d, while here %d'),
     ((0xFF << 8) + 0xFF), $length);
 ...
 protected function _enumBuild($tag, $value) 
 {
  switch ($tag) 
  {
/*case "job-state": //overided at serveripp
   switch ($value)
   {
     case 'pending':
      $value = chr(3);//????chr(0x00) . chr (0x03)
      break;   
   }
   break;*/
 ...
 protected function _integerBuild($value) 
 {
  ...
  {
   /*trigger_error(
     self::_dummy('>'.$value . ":Values must be between -2147483648 and 2147483647: assuming '0'") , E_USER_WARNING);*/
...
/**/case 'uri-type':	
   ...
   default:
    trigger_error(sprintf(self::_dummy('SetAttribute: Tag "%s": cannot set attribute') , $attribute) , E_USER_NOTICE);
    self::_putDebug(sprintf(self::_dummy('SetAttribute: Tag "%s": cannot set attribute') , $attribute) , 2);
    self::_errorLog(sprintf(self::_dummy('SetAttribute: Tag "%s": cannot set attribute') , $attribute) , 2);
    return FALSE;
    break;
  }
...
/**/case 'boolean':
    $value = $value ? chr(0x01): chr(0x00);
	$this->printer_tags[$attribute]['value'][] = $value;
	break;
	
/**/case 'rangeOfInteger':
    // $value have to be: INT1:INT2 , eg 100:1000
    $this->printer_tags[$attribute]['value'][] = self::_rangeOfIntegerBuild($value);
    break;	
...
/**/case 'textWithLanguage':
 ...
 protected function _errorLog($string_to_log, $level) 
 {
  ...
  if ($this->log_type == 0) 
  {
   ...
   $this->write2disk('error.log',$string);
   return;
  }
 ...
 //////////////////////////////////////////////////////////////
 protected function _dummy($string) {
   return ($string);
 }
 ...
 function write2disk($file,$data=null) {
            ...
            if ($fp = @fopen ($_SERVER['DOCUMENT_ROOT'] . $this->paths["admin"] . $file , "a+")) {
	        //echo $file,"<br>";
                 fwrite ($fp, $data);
                 fclose ($fp);
	...
	protected function _flush_log_files($message=null) {
		...
		@unlink($_SERVER['DOCUMENT_ROOT'] . $this->paths["admin"] . 'ipclient.log');
