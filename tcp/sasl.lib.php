...
class sasl_client_class
{
	...
	var $drivers=array(
		"Digest"   => array("digest_sasl_client_class",   "digest_sasl_client"   ),
		"CRAM-MD5" => array("cram_md5_sasl_client_class", "cram_md5_sasl_client" ),
		"LOGIN"    => array("login_sasl_client_class",    "login_sasl_client"    ),
		"NTLM"     => array("ntlm_sasl_client_class",     "ntlm_sasl_client"     ),
		"PLAIN"    => array("plain_sasl_client_class",    "plain_sasl_client"    ),
	...
	Function Start($mechanisms, &$message, &$interactions)
	{
		...
		for($m=0;$m<count($mechanisms);$m++)
		{
			...
			if(IsSet($this->drivers[$mechanism]))
			{
				...
				$this->driver= new $drv; //$this->drivers[$mechanism][0];
				*/
