...
class daemon {
        ...
        function daemon($type='standalone',$prompt=true,$printout=false) {
        ...
        function verbose ($level, $msg) {
                if (VERBOSE && $level <= VERBOSE_LEVEL && SERVER_TYPE != 'inetd') {
				        $message = str_repeat ("*", $level) . " $msg " . str_repeat ("*", $level) . "\n";
                        echo $message;
						
						
						//print
						if (is_resource($this->printout) &&
						    get_resource_type($this->printout)=='printer')
...
declare(ticks=10) {	
                if (SERVER_TYPE == 'inetd') {
                  ...
                  while (true) {
                        ...
                        $ready =  socket_select($current, $this->null, $this->null, $timeout);
                        if ($ready === false) {
                          $this->shutdown();
                        }	
						//echo '2';
