...
class processInst {
	...
	public function nextStep($event=null) {
		if ($this->getProcessStepInfo('isNext')) {
			return $this->getProcessStepInfo('caller') .'.'. $this->getNextInChain() .'.'.
				   ($event ? $event : $this->getProcessStepInfo('event'));	
		}	
	...
	public function prevStep($event=null) { 
		if ($this->getProcessStepInfo('isPrev')) {
			return $this->getProcessStepInfo('name') .'.'. $this->getPrevInChain() .'.'.
				   ($event ? $event : $this->getProcessStepInfo('event'));	
		}	
	...
	public function isFinished($event=null) {
		$this->event = $event;
		/*echo '<br/>Prev step:' . $this->prevStep($event);
		echo '<br/>Step:' . $this->step($event);
		echo '<br/>Next step:' . $this->nextStep($event);*/
	}	
	...
	protected static function _write($data=null) {
   
		if (!$length = strlen($data)) 
			return false;
 
		if ($fp = fopen(self::$processName.'.txt.php', "w")) {	
	...
	protected static function _writeutf8($data=null) {
	...
	protected static function write2disk($data=null) {
